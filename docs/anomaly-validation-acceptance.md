...
- `SENTINEL_INTERNAL_TOKEN`：FastAPI 与长连接进程使用的 canonical 共享令牌；`bootstrap_env.py` 会同时生成同值的 `INTERNAL_EXECUTION_TOKEN`，供 DolphinScheduler Worker 兼容读取。旧部署可继续只提供其中一个变量。同一来源同时提供两个非空且不同的值会安全地启动失败，错误只包含变量名；终端显式环境优先于仓库 `.env`。不要将值写入命令历史、日志或文档。
- `FEISHU_APP_ID` / `FEISHU_APP_SECRET`：同一飞书应用的凭证。不要用命令打印它们。
- `VALIDATION_TIMEOUT_SCAN_INTERVAL_SECONDS`：超时扫描、待重试请求重新入队与卡片收敛周期，默认 `60` 秒。
- `VALIDATION_MAINTENANCE_BATCH_SIZE`：每轮超时扫描、Kafka 重新入队和终态卡片收敛的最大候选数，默认 `50`；未完成项留待下一轮。
- `FEISHU_HTTP_TIMEOUT_SECONDS`：单次飞书 HTTP 调用超时，默认 `10` 秒；维护轮次可在每次外部调用之间取消。
- `KAFKA_BOOTSTRAP_SERVERS`、`KAFKA_ANOMALY_PUSH_TOPIC`、`KAFKA_ANOMALY_PUSH_GROUP`：互动校验与普通通知共用的 Kafka→DolphinScheduler 推送管线配置。
