...
public class StarRocksDynamicTableSinkFactory implements DynamicTableSinkFactory {
    ...
    public DynamicTableSink createDynamicTableSink(Context context, boolean needValidate) {
        ...
        ResolvedSchema resolved = context.getCatalogTable().getResolvedSchema();
        ResolvedSchema physicalSchema = new ResolvedSchema(
                resolved.getColumns().stream().filter(org.apache.flink.table.catalog.Column::isPhysical)
                        .collect(java.util.stream.Collectors.toList()),
                java.util.Collections.emptyList(), resolved.getPrimaryKey().orElse(null));
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        ...
        optionalOptions.add(StarRocksSinkOptions.SINK_PUBLISH_TIMEOUT);
