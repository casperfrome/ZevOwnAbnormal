...
public class StarRocksSink<InputT>
    ...
    public Committer<StarRocksCommittable> createCommitter(CommitterInitContext context) throws IOException {
        return new StarRocksCommitter(sinkOptions, resolveStreamLoadProperties());
    }

    private StreamLoadProperties resolveStreamLoadProperties() {
        if (streamLoadProperties != null) {
            return streamLoadProperties;
        }
        if ("V1".equalsIgnoreCase(sinkOptions.getSinkVersion())) {
            throw new UnsupportedOperationException("Flink 2.3 supports only sink.version=V2 or AUTO");
        }
        com.starrocks.connector.flink.table.sink.StarRocksSinkFactory.detectStarRocksFeature(sinkOptions);
        if (sinkOptions.getSemantic() == com.starrocks.connector.flink.table.sink.StarRocksSinkSemantic.EXACTLY_ONCE
                && !sinkOptions.isSupportTransactionStreamLoad()) {
            throw new IllegalStateException("Exactly-once requires StarRocks transaction stream load");
        }
        return sinkOptions.getProperties(null);
    }
