...
public class StarRocksCommitter implements Committer<StarRocksCommittable> {
    ...
    public StarRocksCommitter(
        ...
        this.retryIntervalMs = Math.max(0, sinkOptions.getRetryIntervalMs());
    ...
    public void commit(Collection<CommitRequest<StarRocksCommittable>> committables)
        ...
        for (CommitRequest<StarRocksCommittable> commitRequest : committables) {
            ...
            if (!committed) {
                // Fail the checkpoint so Flink restores and retries the persisted committable.
                // Returning normally here acknowledges a transaction that may never have committed.
                throw new IOException("Failed to commit StarRocks transaction after "
