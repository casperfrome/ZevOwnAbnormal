...
public class StarRocksWriterStateSerializer implements SimpleVersionedSerializer<StarRocksWriterState> {
    ...
    public StarRocksWriterState deserialize(int version, byte[] serialized) throws IOException {
        if (version != getVersion()) {
            throw new IOException("Unsupported StarRocks writer state version: " + version);
        }
        try {
            StarRocksWriterState state = jsonWrapper.parseObject(serialized, StarRocksWriterState.class);
            if (state == null || state.getLabelSnapshots() == null) {
                throw new IOException("Missing label snapshots in StarRocks writer state");
            }
            return state;
        } catch (RuntimeException e) {
            throw new IOException("Invalid StarRocks writer state", e);
        }
    }
}
