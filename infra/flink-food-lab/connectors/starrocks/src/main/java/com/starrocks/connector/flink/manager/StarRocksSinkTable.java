    ...
    public void validateTableStructure(StarRocksSinkOptions sinkOptions, ResolvedSchema flinkSchema) {
        ...
        if (!primaryKeys.isEmpty()) {
            if (!constraint.isPresent()) {
                throw new IllegalArgumentException("Primary keys not defined in the sink resolved schema.");
            }
            if (constraint.get().getColumns().size() != primaryKeys.size() ||
                ...
                throw new IllegalArgumentException("Primary keys of the Flink resolved schema do not match the StarRocks table.");
            }
