import { Component, Suspense, lazy, type ErrorInfo, type ReactNode, useCallback, useEffect, useMemo, useState } from "react"
...
function AppShell({ resources, user, setUser }: { resources: ReturnType<typeof createResources>; user: User; setUser: (user: User | null) => void }) {
  ...
  return <AppContext.Provider value={{ resources, user, setUser }}><SidebarProvider><Sidebar collapsible="icon"><SidebarHeader><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" onClick={() => navigate("#records")} tooltip="ZevOwnAbnormal"><span className="grid size-8 place-items-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground"><AlertTriangle /></span><span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-semibold">ZevOwnAbnormal</span><span className="truncate text-xs">异常运营平台</span></span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarHeader><SidebarContent>{navGroups.map((group) => <SidebarGroup key={group.label}><SidebarGroupLabel>{group.label}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{group.items.filter((item) => !item.admin || user.is_superuser).map((item) => <SidebarMenuItem key={item.page}><SidebarMenuButton asChild isActive={route.page === item.page || (item.page === "rules" && route.page === "rule-editor") || (item.page === "datasets" && route.page === "dataset-editor")} tooltip={item.label}><a href={`#${item.page}`}><item.icon /><span>{item.label}</span></a></SidebarMenuButton>{item.page === "records" && <SidebarMenuBadge>•</SidebarMenuBadge>}</SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>)}</SidebarContent><SidebarFooter><SidebarMenu><SidebarMenuItem><DropdownMenu><DropdownMenuTrigger asChild><SidebarMenuButton size="lg"><Avatar className="size-8"><AvatarFallback>{initials(user.display_name)}</AvatarFallback></Avatar><span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-medium">{user.display_name}</span><span className="truncate text-xs">{user.is_superuser ? "超级管理员" : user.job_title || "普通账号"}</span></span><ChevronsUpDown /></SidebarMenuButton></DropdownMenuTrigger><DropdownMenuContent side="top" align="end" className="w-56"><DropdownMenuLabel>{user.login_name}</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => navigate("#account")}><CircleUserRound data-icon="inline-start" />个人账号</DropdownMenuItem><DropdownMenuItem onSelect={async () => { await resources.auth.logout(); setUser(null) }}><LogOut data-icon="inline-start" />退出登录</DropdownMenuItem></DropdownMenuContent></DropdownMenu></SidebarMenuItem></SidebarMenu></SidebarFooter><SidebarRail /></Sidebar><SidebarInset><header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur"><SidebarTrigger /><Breadcrumb><BreadcrumbList><BreadcrumbItem>ZevOwnAbnormal</BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{titles[route.page]}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb><div className="ml-auto"><Button variant="outline" className="hidden w-64 justify-start text-muted-foreground sm:flex" onClick={() => setSearchOpen(true)}><Search data-icon="inline-start" />搜索规则、数据集…<kbd className="ml-auto text-xs">Ctrl K</kbd></Button><Button variant="ghost" size="icon" className="sm:hidden" aria-label="打开全局搜索" onClick={() => setSearchOpen(true)}><Search /></Button></div></header><main className="mx-auto w-full max-w-[1600px] flex-1 p-4 md:p-6">{content}</main></SidebarInset></SidebarProvider><CommandDialog open={searchOpen} onOpenChange={setSearchOpen}><Command><CommandInput placeholder="搜索规则、数据集或页面…" /><CommandList><CommandEmpty>没有匹配结果</CommandEmpty><CommandGroup heading="页面">{navGroups.flatMap((group) => group.items).filter((item) => !item.admin || user.is_superuser).map((item) => <CommandItem key={item.page} onSelect={() => selectSearch(`#${item.page}`)}><item.icon />{item.label}</CommandItem>)}</CommandGroup><CommandGroup heading="规则">{Array.isArray(rules.data) && rules.data.map((rule) => <CommandItem key={rule.id} onSelect={() => selectSearch(`#rules/${rule.id}/edit`)}><ShieldCheck />{rule.name}<CommandShortcut>规则</CommandShortcut></CommandItem>)}</CommandGroup><CommandGroup heading="数据集">{Array.isArray(datasets.data) && datasets.data.map((dataset) => <CommandItem key={dataset.id} onSelect={() => selectSearch(`#datasets/${dataset.id}/edit`)}><Server />{dataset.name}<CommandShortcut>数据集</CommandShortcut></CommandItem>)}</CommandGroup></CommandList></Command></CommandDialog></AppContext.Provider>
}
