...
import { ErrorState, IconTooltip, LoadingState } from "@/components/shared"
...
const navGroups = [
  ...
  { label: "系统", items: [{ page: "overview", label: "总览", icon: BarChart3 }, { page: "tests", label: "系统测试", icon: FlaskConical, admin: true }, { page: "accounts", label: "账号管理", icon: Users, admin: true }] },
]
...
function AppShell({ resources, user, setUser }: { resources: ReturnType<typeof createResources>; user: User; setUser: (user: User | null) => void }) {
  const { hash, navigate } = useHash(); const route = parseHashRoute(hash); const canManage = user.is_superuser; const [searchOpen, setSearchOpen] = useState(false); const searchTriggerRef = useRef<HTMLElement | null>(null)
  const rules = useQuery({ queryKey: ["rules"], queryFn: ({ signal }) => resources.rules.list(signal), staleTime: 60_000, enabled: canManage }); const datasets = useQuery({ queryKey: ["datasets"], queryFn: ({ signal }) => resources.datasets.list(signal), staleTime: 60_000, enabled: canManage })
  ...
  useEffect(() => { const listener = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") { event.preventDefault(); searchTriggerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; setSearchOpen((value) => !value) } }; window.addEventListener("keydown", listener); return () => window.removeEventListener("keydown", listener) }, [])
  useEffect(() => {
    if (canManage) return
    if (route.page === "tests" || route.page === "accounts") navigate("#records", { replace: true })
    else if (route.page === "rule-editor") navigate("#rules", { replace: true })
    else if (route.page === "dataset-editor") navigate("#datasets", { replace: true })
  }, [canManage, navigate, route.page])
  const selectSearch = (target: string) => { setSearchOpen(false); navigate(target) }
  const content = <Suspense fallback={<LoadingState label="正在加载页面" />}>{(() => { switch (route.page) { case "records": return <RecordsPage detailId={route.detailId} navigate={navigate} />; case "anomaly-groups": return <GroupsPage detailId={route.detailId} navigate={navigate} />; case "rules": return <RulesPage navigate={navigate} />; case "rule-editor": return canManage ? <RuleEditorPage id={route.entityId} navigate={navigate} /> : <RulesPage navigate={navigate} />; case "datasets": return <DatasetsPage navigate={navigate} />; case "dataset-editor": return canManage ? <DatasetEditorPage id={route.entityId} navigate={navigate} /> : <DatasetsPage navigate={navigate} />; case "datasources": return <DatasourcesPage />; case "overview": return <OverviewPage />; case "tests": return canManage ? <TestsPage /> : <RecordsPage navigate={navigate} />; case "account": return <AccountPage />; case "accounts": return canManage ? <AccountsPage /> : <RecordsPage navigate={navigate} /> } })()}</Suspense>
  return <AppContext.Provider value={{ resources, user, canManage, setUser }}><SidebarProvider style={{ "--sidebar-width": "248px" } as CSSProperties}><Sidebar className="app-sidebar" collapsible="offcanvas"><SidebarHeader><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" onClick={() => navigate("#records")} tooltip="ZevOwnAbnormal"><span className="grid size-9 place-items-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground"><AlertTriangle /></span><span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-semibold">ZevOwnAbnormal</span><span className="truncate text-xs">异常运营平台</span></span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarHeader><SidebarContent>{navGroups.map((group) => <SidebarGroup key={group.label}><SidebarGroupLabel>{group.label}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{group.items.filter((item) => !item.admin || canManage).map((item) => <SidebarMenuItem key={item.page}><SidebarMenuButton className="app-nav-item" asChild isActive={route.page === item.page || (item.page === "rules" && route.page === "rule-editor") || (item.page === "datasets" && route.page === "dataset-editor")} tooltip={item.label}><a href={`#${item.page}`}><item.icon /><span>{item.label}</span></a></SidebarMenuButton>{item.page === "records" && pendingRecords.data && pendingRecords.data > 0 ? <SidebarMenuBadge aria-label={`${pendingRecords.data} 条待处理异常`}>{pendingRecords.data}</SidebarMenuBadge> : null}</SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>)}</SidebarContent><SidebarFooter><SidebarMenu><SidebarMenuItem><DropdownMenu><DropdownMenuTrigger asChild><SidebarMenuButton size="lg"><Avatar className="size-8"><AvatarFallback>{initials(user.display_name)}</AvatarFallback></Avatar><span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-medium">{user.display_name}</span><span className="truncate text-xs">{canManage ? "超级管理员" : user.job_title || "普通账号"}</span></span><ChevronsUpDown /></SidebarMenuButton></DropdownMenuTrigger><DropdownMenuContent side="top" align="end" className="w-56"><DropdownMenuLabel>{user.login_name}</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => navigate("#account")}><CircleUserRound data-icon="inline-start" />个人账号</DropdownMenuItem><DropdownMenuItem onSelect={async () => { await resources.auth.logout(); setUser(null) }}><LogOut data-icon="inline-start" />退出登录</DropdownMenuItem></DropdownMenuContent></DropdownMenu></SidebarMenuItem></SidebarMenu></SidebarFooter><SidebarRail /></Sidebar><SidebarInset><header className="app-topbar sticky top-0 z-20 flex h-[72px] items-center gap-4 border-b bg-background/95 px-4 backdrop-blur md:px-8"><IconTooltip label="打开导航"><SidebarTrigger aria-label="打开导航" /></IconTooltip><Breadcrumb className="min-w-0"><BreadcrumbList><BreadcrumbItem>ZevOwnAbnormal</BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{titles[route.page]}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb><div className="ml-auto"><Button variant="outline" className="hidden w-70 justify-start text-muted-foreground sm:flex" onClick={(event) => { searchTriggerRef.current = event.currentTarget; setSearchOpen(true) }}><Search data-icon="inline-start" />搜索规则、数据集…<kbd className="ml-auto text-xs">Ctrl K</kbd></Button><IconTooltip label="打开全局搜索"><Button variant="ghost" size="icon" className="sm:hidden" aria-label="打开全局搜索" onClick={(event) => { searchTriggerRef.current = event.currentTarget; setSearchOpen(true) }}><Search /></Button></IconTooltip></div></header><main className="app-content mx-auto w-full max-w-[1440px] flex-1 p-4 md:p-8">{content}</main></SidebarInset></SidebarProvider><CommandDialog open={searchOpen} onOpenChange={setSearchOpen} onCloseAutoFocus={(event) => { if (searchTriggerRef.current) { event.preventDefault(); searchTriggerRef.current.focus() } }}><Command><CommandInput placeholder="搜索规则、数据集或页面…" /><CommandList><CommandEmpty>没有匹配结果</CommandEmpty><CommandGroup heading="页面">{navGroups.flatMap((group) => group.items).filter((item) => !item.admin || canManage).map((item) => <CommandItem key={item.page} onSelect={() => selectSearch(`#${item.page}`)}><item.icon />{item.label}</CommandItem>)}</CommandGroup>{canManage && <><CommandGroup heading="规则">{Array.isArray(rules.data) && rules.data.map((rule) => <CommandItem key={rule.id} onSelect={() => selectSearch(`#rules/${rule.id}/edit`)}><ShieldCheck />{rule.name}<CommandShortcut>规则</CommandShortcut></CommandItem>)}</CommandGroup><CommandGroup heading="数据集">{Array.isArray(datasets.data) && datasets.data.map((dataset) => <CommandItem key={dataset.id} onSelect={() => selectSearch(`#datasets/${dataset.id}/edit`)}><Server />{dataset.name}<CommandShortcut>数据集</CommandShortcut></CommandItem>)}</CommandGroup></>}</CommandList></Command></CommandDialog></AppContext.Provider>
}
