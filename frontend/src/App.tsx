...
function AppShell({ resources, user, setUser }: { resources: ReturnType<typeof createResources>; user: User; setUser: (user: User | null) => void }) {
  const { hash, navigate } = useHash(); const route = parseHashRoute(hash); const [searchOpen, setSearchOpen] = useState(false); const searchTriggerRef = useRef<HTMLElement | null>(null)
