...
window.DatasourceModule = (function () {
  ...
  function renderActions(actionsEl) {
    ...
    actionsEl.querySelector('#ds-refresh').addEventListener('click', () => {
      UI.toast({ type: 'info', title: '已刷新数据源列表', desc: '检测时间已更新' });
      renderList();
    });
  }
  ...
  function renderStats() {
    ...
    document.getElementById('ds-stats').innerHTML = `
      <div class="stat-card animate-rise" style="animation-delay:60ms;">
        ...
        <div class="stat-card-delta neutral">${Icon.users({ size: 12 })}<span>覆盖 2 种类型</span></div>
      </div>
    ...
    const m = UI.modal({
      ...
      body: `
        <div class="form-section">
          ...
          <div class="form-section-desc">数据源的类型与显示名称</div>
          <div class="form-grid">
          ...
          <div class="field">
            ...
            <div class="radio-cards" style="grid-template-columns: 1fr 1fr;">
              <div class="radio-card ${data.type === 'mysql' ? 'selected' : ''}" data-type="mysql">
              ...
              <div class="radio-card ${data.type === 'starrocks' ? 'selected' : ''}" data-type="starrocks">
        ...
        <div class="form-section">
          ...
          <div class="form-grid">
            ...
            ${UI.field('密码', `<div class="input-group"><input class="input mono" id="f-password" type="password" value="${escapeHtml(data.password || '')}" placeholder="输入密码" style="border:none;box-shadow:none;" /><button type="button" class="input-adornment right" id="f-toggle-pw" style="cursor:pointer;color:var(--color-ink-muted);">${Icon.eye({ size: 16 })}</button></div>`, { optional: true })}
    ...
    m.dialog.querySelector('#f-test').addEventListener('click', async () => {
      ...
      try {
        ...
        await Store.testDatasourceConfig({
          ...
          password: m.dialog.querySelector('#f-password').value, ssl: m.dialog.querySelector('#f-ssl').checked,
        });
